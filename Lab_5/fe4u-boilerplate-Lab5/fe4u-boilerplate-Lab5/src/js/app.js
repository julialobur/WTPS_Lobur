...
async function setup() {
  ...
  function renderPagination() {
    ...
    const lastPage = Math.ceil(totalTeachers / limit);
    const pagingContainer = document.querySelector(".paging");
    pagingContainer.innerHTML = "";

    // Додаємо кнопку "Previous" (якщо не перша сторінка)
    if (currentPage > 1) {
      const prevButton = document.createElement("span");
      prevButton.classList.add("paging__button");
    ...
    for (let i = startPage; i <= endPage; i++) {
      ...
      pageButton.classList.add("paging__button");
      if (i === currentPage) {
    ...
    // Додаємо кнопку "Next" (якщо не остання сторінка)
    if (currentPage < lastPage) {
      const nextButton = document.createElement("span");
      nextButton.classList.add("paging__button");
    ...
    // Додаємо кнопку "Last" (якщо не остання сторінка і не показана)
    if (endPage < lastPage) {
      ...
      lastButton.classList.add("paging__button");
