...
async function setup() {
  ...
  function openTeacherForm(user) {
    ...
    if (!user.birthDate || !dayjs(user.birthDate).isValid()) {
      if (teacherDaysToBirthday) {
        teacherDaysToBirthday.innerText = "Birthday info unavailable";
      }
    } else {
      const birthDate = dayjs(user.birthDate);
      const birtdayInThisYear = birthDate.year(currentDate.year());
      const nextBirthday = birtdayInThisYear.isAfter(currentDate)
        ? birtdayInThisYear
        : birtdayInThisYear.add(1, "year");
      const daysToBirthday = nextBirthday.diff(currentDate, "day");

      if (teacherDaysToBirthday) {
        teacherDaysToBirthday.innerText = `Days to birthday: ${daysToBirthday}`;
      }
    }
    ...
    teacherPersonFavorite.innerText = user.favorite ? '★' : '☆';
    teacherPersonFavorite.style.fontSize = '28px';
    teacherPersonFavorite.style.color = 'yellow';
  ...
  const renderFavoriteTeachers = () => {
    ...
    const currentFavoriteTeachers = filterPeople(users, { favorite: true }).slice(0, 5); // Only take first 5
  ...
  addMoreTeachersBtn.onclick = async () => {
    ...
    renderTeachers();
    renderFavoriteTeachers();
    const activeChart = chartOptions.find(({ node }) =>
      node.classList.contains("btn--active")
    );
    if (activeChart) {
      const labels = users.map((user) => user[activeChart.key]);
      const uniqLabels = _.uniq(labels);
      const data = uniqLabels.map(
        (label) => users.filter((user) => user[activeChart.key] === label).length
      );
